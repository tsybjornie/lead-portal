...
export default function DrafterSignup() {
    const [form, setForm] = useState({ name: '', email: '', phone: '', portfolio: '', software: '', experience: '', rate: '', notes: '' });
    ...
    const handleSubmit = async () => {
        if (!form.name || !form.phone || !form.software) return;
        setIsSubmitting(true);
        setMessage('');
        try {
            const res = await fetch('/api/submit/drafter', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    full_name: form.name, email: form.email, phone: form.phone,
                    portfolio_url: form.portfolio, primary_software: form.software,
                    experience: form.experience, hourly_rate: form.rate, notes: form.notes,
                }),
            });
            const data = await res.json();
            if (!res.ok) { setMessage(data.error || 'Something went wrong.'); setMessageType('error'); }
            else {
                setMessage('Submitted! We\'ll review your portfolio.'); setMessageType('success'); setForm({ name: '', email: '', phone: '', portfolio: '', software: '', experience: '', rate: '', notes: '' });
            }
        ...
        <div style={{ fontFamily: f, background: '#fafafa', minHeight: '100vh', color: '#111' }}>
            ...
            <nav style={{ padding: '0 48px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
                    ...
                    <span style={{ fontFamily: mono, fontSize: 10, fontWeight: 500, color: 'rgba(0,0,0,0.4)', letterSpacing: '0.1em', textTransform: 'uppercase' as const }}>DRAFTER</span>
                </div>
            ...
            <div style={{ maxWidth: 560, margin: '0 auto', padding: '80px 48px 80px' }}>
                <div style={{ fontFamily: mono, fontSize: 10, fontWeight: 500, color: 'rgba(0,0,0,0.45)', letterSpacing: '0.2em', textTransform: 'uppercase' as const, marginBottom: 24 }}>DRAFTER / 3D ARTIST REGISTRATION</div>
                <h1 style={{ fontSize: 32, fontWeight: 300, letterSpacing: '-0.03em', margin: '0 0 8px', color: '#111' }}>
                    Draw it.<br /><span style={{ color: 'rgba(0,0,0,0.3)', fontStyle: 'italic' }}>render it.</span>
                </h1>
                <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.5)', margin: '0 0 48px', lineHeight: 1.6 }}>
                    Join Roof's drafter network. Get matched with design firms who need SketchUp models, shop drawings, renders, and 3D visualisations — on demand.
                </p>
                ...
                <div style={{ marginBottom: 32 }}>
                    ...
                    <div style={{ marginBottom: 16 }}><label style={labelStyle}>Email</label><input style={inputStyle} type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} placeholder="you@email.com" /></div>
                    <div><label style={labelStyle}>Portfolio Link</label><input style={inputStyle} value={form.portfolio} onChange={e => setForm(p => ({ ...p, portfolio: e.target.value }))} placeholder="Behance, Dribbble, or personal site" /></div>
                </div>

                {/* SKILLS */}
                <div style={{ marginBottom: 32 }}>
                    <div style={sectionHeader}>SKILLS & SOFTWARE</div>
                    <div style={{ marginBottom: 16 }}>
                        <label style={labelStyle}>Primary Software</label>
                        <select style={{ ...selectStyle, color: form.software ? '#111' : 'rgba(0,0,0,0.3)' }} value={form.software} onChange={e => setForm(p => ({ ...p, software: e.target.value }))}>
                            <option value="">Select your main tool</option>
                            {SOFTWARE.map(t => <option key={t} value={t}>{t}</option>)}
                        </select>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                        <div><label style={labelStyle}>Years of Experience</label><input style={inputStyle} value={form.experience} onChange={e => setForm(p => ({ ...p, experience: e.target.value }))} placeholder="e.g. 5" /></div>
                        <div><label style={labelStyle}>Rate (S$/project)</label><input style={inputStyle} value={form.rate} onChange={e => setForm(p => ({ ...p, rate: e.target.value }))} placeholder="e.g. 350" /></div>
                    </div>
                </div>

                {/* SAMPLE WORK */}
                <div style={{ marginBottom: 32 }}>
                    <div style={sectionHeader}>SAMPLE WORK</div>
                    <label style={labelStyle}>Upload renders or drawings</label>
                    <p style={{ fontSize: 11, color: 'rgba(0,0,0,0.35)', margin: '0 0 12px', lineHeight: 1.4 }}>
                        SketchUp renders, shop drawings, floor plans — PNG, JPG, or PDF. Max 5 files, 10MB each.
                    </p>
                    <label style={{
                        ...
                        <span style={{ fontSize: 12, color: 'rgba(0,0,0,0.4)', fontWeight: 500 }}>
                            {sampleFiles.length > 0 ? 'Add more files' : 'Click to upload sample work'}
                        </span>
                        <input
                            type="file"
                            ...
                            onChange={e => {
                                ...
                                setSampleFiles(prev => [...prev, ...valid].slice(0, 5));
                                e.target.value = '';
                            }}
                        />
                    </label>
                        ...
                        <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
                            {sampleFiles.map((file, i) => (
                                <div key={i} style={{
                                    ...
                                    <button type="button" onClick={() => setSampleFiles(prev => prev.filter((_, j) => j !== i))} style={{
                ...
                <div style={{ marginBottom: 32 }}>
                    ...
                    <textarea style={{ ...inputStyle, height: 80, resize: 'vertical' as const }} value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="Availability, specialisations, preferred project types..." />
                </div>

                <button onClick={handleSubmit} disabled={isSubmitting || !form.name || !form.phone || !form.software} style={{ width: '100%', padding: '14px', fontSize: 13, fontWeight: 600, fontFamily: f, color: '#fff', background: isSubmitting ? 'rgba(0,0,0,0.3)' : '#111', border: 'none', borderRadius: 6, cursor: isSubmitting ? 'not-allowed' : 'pointer', transition: 'background 0.2s' }}>
                    {isSubmitting ? 'Submitting...' : 'Register as Drafter →'}
                </button>
            ...
            <footer style={{ padding: '28px 48px', borderTop: '1px solid rgba(0,0,0,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontFamily: mono, fontSize: 10, color: 'rgba(0,0,0,0.35)', letterSpacing: '0.05em' }}>© 2026 ROOF · DRAFTER NETWORK</span>
                <div style={{ display: 'flex', gap: 24 }}>
                    {[{ label: 'Landing', href: '/landing' }, { label: 'Log in', href: '/login' }].map(link => (
